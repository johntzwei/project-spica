// Run with `bun scripts/generate-mosaic.ts`. No runtime dependencies.
// Keep the ceramic composition; change only the tesserae, not the landscape.
const width = 1400;
const height = 350;
const tileSize = 8.75; // Same small scale as the main worktree.
const sun = { x: 1045, y: 96, radius: 63 };
let seed = 73921;
const random = () => {
...
// These are the original horizon and ribbon boundaries, without rescaling.
const horizon = (x: number) => 211 - 30 * Math.sin(x / 240) + 9 * Math.cos(x / 113);
const fieldY = (x: number, row: number) => horizon(x) + row * (17 + 7 * Math.sin(x / 320 + 1) ** 2);
...
let radius = 0;
for (let ring = 0; radius < 1140; ring++) {
...
for (let attempt = 0; attempt < 1000 && rightStars.length < 8; attempt++) {
...
const fieldOutline = `M-20 ${horizon(-20)} ${Array.from({ length: 145 }, (_, i) => {
  const x = -20 + i * 10;
...
const render = (night: boolean) => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1400 350" role="img" aria-labelledby="title desc">
  ...
  <defs>
    <clipPath id="frame"><rect width="1400" height="350"/></clipPath>
  ...
  <g clip-path="url(#frame)" filter="url(#stone)">
    <path fill="${night ? "#0a142b" : "#702333"}" d="M0 0H1400V350H0Z"/>
    ...
    <rect width="1400" height="350" fill="url(#light)"/>
  </g>
</svg>\n`;
await Bun.write(new URL("../public/images/spica-mosaic.svg", import.meta.url), render(false));
await Bun.write(new URL("../public/images/spica-mosaic-night.svg", import.meta.url), render(true));
// The responsive title moves relative to the cropped image. Export the exact
// fitted sky geometry so its constellation can recolor tiles beneath the i,
// rather than translating a separate constellation across the grout.
await Bun.write(new URL("../public/images/sky-tiles.json", import.meta.url), JSON.stringify(skyTiles.map(tile => ({
...
console.log(`Generated day and night ceramic artwork with ${tiles.length.toLocaleString()} small landscape tiles each.`);
